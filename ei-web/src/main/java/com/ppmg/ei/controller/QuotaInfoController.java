...
@Scope("prototype")
@Api(tags={"经营计划（指标接口"})
public class QuotaInfoController extends BaseAction{
...
/*	protected final Logger logger = LoggerFactory.getLogger(this.getClass());
	...
	public String list(@RequestParam("length") int length, @RequestParam("currPage") int currPage){
		try {
			...
			PageInfo<QuotaInfoModel> rows = quotaInfoService.selectListPage(searchCondition);
			List<BaseVO> list = new ArrayList<BaseVO>();
			for(QuotaInfoModel model : rows.getList()){
	...
	public String detail(@PathVariable(value = "id") String id){
		try {
			QuotaInfoModel model = quotaInfoService.selectById(id);
			BaseVO vo = new BaseVO();
			BeanUtils.copyProperties(vo, model);
