...
@Scope("prototype")
@Api(tags={"XXX接口"})
@RequestMapping("tzTCooConditionFile")
public class TzTCooConditionFileController extends BaseAction{

	protected final Logger logger = LoggerFactory.getLogger(this.getClass());

	@Reference
	private TzTCooConditionFileService tzTCooConditionFileService;
	...
	public String list(@RequestParam("length") int length, @RequestParam("currPage") int currPage){
		try {
			...
			PageInfo<TzTCooConditionFileModel> rows = tzTCooConditionFileService.selectListPage(searchCondition);
			List<BaseVO> list = new ArrayList<BaseVO>();
			for(TzTCooConditionFileModel model : rows.getList()){
	...
	public String detail(@PathVariable(value = "id") String id){
		try {
			TzTCooConditionFileModel model = tzTCooConditionFileService.selectById(id);
			BaseVO vo = new BaseVO();
