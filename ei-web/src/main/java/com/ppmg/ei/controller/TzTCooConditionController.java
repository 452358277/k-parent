...
@Scope("prototype")
@Api(tags={"XXX接口"})
@RequestMapping("tzTCooCondition")
public class TzTCooConditionController extends BaseAction{

	protected final Logger logger = LoggerFactory.getLogger(this.getClass());

	@Reference(check = false)
	private TzTCooConditionService tzTCooConditionService;
	...
	public String list(@RequestParam("length") int length, @RequestParam("currPage") int currPage){
		try {
			...
			PageInfo<TzTCooConditionModel> rows = tzTCooConditionService.selectListPage(searchCondition);
			List<BaseVO> list = new ArrayList<BaseVO>();
			for(TzTCooConditionModel model : rows.getList()){
	...
	public String detail(@PathVariable(value = "id") String id){
		try {
			TzTCooConditionModel model = tzTCooConditionService.selectById(id);
			BaseVO vo = new BaseVO();
