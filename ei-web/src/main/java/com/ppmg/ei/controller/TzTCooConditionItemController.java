...
@Scope("prototype")
@Api(tags={"XXX接口"})
@RequestMapping("tzTCooConditionItem")
public class TzTCooConditionItemController extends BaseAction{

	protected final Logger logger = LoggerFactory.getLogger(this.getClass());

	@Reference
	private TzTCooConditionItemService tzTCooConditionItemService;
	...
	public String list(@RequestParam("length") int length, @RequestParam("currPage") int currPage){
		try {
			...
			PageInfo<TzTCooConditionItemModel> rows = tzTCooConditionItemService.selectListPage(searchCondition);
			List<BaseVO> list = new ArrayList<BaseVO>();
			for(TzTCooConditionItemModel model : rows.getList()){
	...
	public String detail(@PathVariable(value = "id") String id){
		try {
			TzTCooConditionItemModel model = tzTCooConditionItemService.selectById(id);
			BaseVO vo = new BaseVO();
