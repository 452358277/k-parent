...
@Scope("prototype")
@Api(tags={"XXX接口"})
public class AppUserroleController extends BaseAction{
...
/*	@ApiOperation(value="XXX列表", notes="XXX列表")
	...
	public String list(@RequestParam("length") int length, @RequestParam("currPage") int currPage){
		try {
			...
			PageInfo<AppUserroleModel> rows = appUserroleService.selectListPage(searchCondition);
			List<BaseVO> list = new ArrayList<BaseVO>();
			for(AppUserroleModel model : rows.getList()){
	...
	public String detail(@PathVariable(value = "id") String id){
		try {
			AppUserroleModel model = appUserroleService.selectById(id);
			BaseVO vo = new BaseVO();
			BeanUtils.copyProperties(vo, model);
