...
public class AdminStudyController extends BaseAction{
	...
	public String list(@RequestParam("length") int length, @RequestParam("currPage") int currPage){
		try {
			...
			PageInfo<AdminStudyModel> rows = adminStudyService.selectListPage(searchCondition);
			List<BaseVO> list = new ArrayList<BaseVO>();
			for(AdminStudyModel model : rows.getList()){
	...
	public String detail(@PathVariable(value = "id") String id){
		try {
			AdminStudyModel model = adminStudyService.selectById(id);
			BaseVO vo = new BaseVO();
