...
@Scope("prototype")
@Api(tags={"XXX接口"})
@RequestMapping("tzTCooConditionInfo")
public class TzTCooConditionInfoController extends BaseAction{
	...
	public String list(@RequestParam("length") int length, @RequestParam("currPage") int currPage){
		try {
			...
			PageInfo<TzTCooConditionInfoModel> rows = tzTCooConditionInfoService.selectListPage(searchCondition);
			List<BaseVO> list = new ArrayList<BaseVO>();
			for(TzTCooConditionInfoModel model : rows.getList()){
	...
	public String detail(@PathVariable(value = "id") String id){
		try {
			TzTCooConditionInfoModel model = tzTCooConditionInfoService.selectById(id);
			BaseVO vo = new BaseVO();
