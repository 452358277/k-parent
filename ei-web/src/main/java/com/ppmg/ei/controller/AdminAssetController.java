...
public class AdminAssetController extends BaseAction{
	...
	public String list(@RequestParam("length") int length, @RequestParam("currPage") int currPage){
		try {
			...
			PageInfo<AdminAssetModel> rows = adminAssetService.selectListPage(searchCondition);
			List<BaseVO> list = new ArrayList<BaseVO>();
			for(AdminAssetModel model : rows.getList()){
	...
	public String detail(@PathVariable(value = "id") String id){
		try {
			AdminAssetModel model = adminAssetService.selectById(id);
			BaseVO vo = new BaseVO();
