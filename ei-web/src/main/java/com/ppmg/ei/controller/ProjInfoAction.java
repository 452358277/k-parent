...
@Scope("prototype")
@Api(tags={"XXX接口"})
public class ProjInfoAction extends BaseAction{
...
/*	@ApiOperation(value="XXX列表", notes="XXX列表")
	...
	@ResponseBody
	public String list(@RequestParam("pageSize") int pageSize, @RequestParam("currPage") int currPage){
		try {
	...
	public String detail(@PathVariable(value = "id") String id){
		try {
			ProjInfoModel model = projInfoService.selectById(id);
			BaseVO vo = new BaseVO();
			BeanUtils.copyProperties(vo, model);
