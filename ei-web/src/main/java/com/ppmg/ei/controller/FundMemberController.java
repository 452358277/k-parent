...
@Scope("prototype")
@Api(tags={"XXX接口"})
public class FundMemberController extends BaseAction{
	...
	public String list(@RequestParam("length") int length, @RequestParam("currPage") int currPage){
		try {
			...
			PageInfo<FundMemberModel> rows = fundMemberService.selectListPage(searchCondition);
			List<BaseVO> list = new ArrayList<BaseVO>();
			for(FundMemberModel model : rows.getList()){
	...
	public String detail(@PathVariable(value = "id") String id){
		try {
			FundMemberModel model = fundMemberService.selectById(id);
			BaseVO vo = new BaseVO();
			BeanUtils.copyProperties(vo, model);
