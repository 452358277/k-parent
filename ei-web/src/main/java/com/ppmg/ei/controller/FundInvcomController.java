...
import com.ppmg.ei.vo.FundInvcomVO;
...
@Scope("prototype")
@Api(tags={"基金内部机构接口"})
public class FundInvcomController extends BaseAction{
	...
	public String list(@RequestParam("length") int length, @RequestParam("currPage") int currPage){
		try {
			...
			PageInfo<FundInvcomModel> rows = fundInvcomService.selectListPage(searchCondition);
			List<BaseVO> list = new ArrayList<BaseVO>();
			for(FundInvcomModel model : rows.getList()){
	...
	public String detail(@PathVariable(value = "id") String id){
		try {
			FundInvcomModel model = fundInvcomService.selectById(id);
			BaseVO vo = new BaseVO();
			BeanUtils.copyProperties(vo, model);
