...
@Scope("prototype")
@Api(tags={"经营计划接口"})
public class QuotaConfDetailController extends BaseAction{
	...
	@GetMapping(value = "/QuotaConfDetailList", produces = "application/json;charset=UTF-8")
	@ResponseBody
	public String list(@RequestParam("length") int length, @RequestParam("currPage") int currPage){
		try {
			...
			PageInfo<QuotaConfDetailModel> rows = quotaConfDetailService.selectListPage(searchCondition);
			List<BaseVO> list = new ArrayList<BaseVO>();
			for(QuotaConfDetailModel model : rows.getList()){
	...
	@ApiOperation(value="获取经营计划详细信息", notes="根据url的id来获取经营计划详细信息")
	@ApiImplicitParam(name = "id", value = "QuotaConfDetailID", required = true, dataType = "String", paramType = "path")
	@ApiResponses({ @ApiResponse(code = CommonStatus.OK, message = "操作成功"),
        @ApiResponse(code = CommonStatus.EXCEPTION, message = "服务器内部异常"),
   })
	@GetMapping(value = "/QuotaConfDetailInfo/{id}", produces = "application/json;charset=UTF-8")
	@ResponseBody
	public String detail(@PathVariable(value = "id") String id){
		try {
			QuotaConfDetailModel model = quotaConfDetailService.selectById(id);
			BaseVO vo = new BaseVO();
			BeanUtils.copyProperties(vo, model);
