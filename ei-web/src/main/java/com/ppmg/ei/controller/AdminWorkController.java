...
public class AdminWorkController extends BaseAction{
	...
	public String list(@RequestParam("length") int length, @RequestParam("currPage") int currPage){
		try {
			...
			PageInfo<AdminWorkModel> rows = adminWorkService.selectListPage(searchCondition);
			List<BaseVO> list = new ArrayList<BaseVO>();
			for(AdminWorkModel model : rows.getList()){
	...
	public String detail(@PathVariable(value = "id") String id){
		try {
			AdminWorkModel model = adminWorkService.selectById(id);
			BaseVO vo = new BaseVO();
