...
@Scope("prototype")
@Api(tags={"XXX接口"})
@RequestMapping("adminCore")
public class AdminCoreController extends BaseAction{
	...
	public String list(@RequestParam("length") int length, @RequestParam("currPage") int currPage){
		try {
			...
			PageInfo<AdminCoreModel> rows = adminCoreService.selectListPage(searchCondition);
			List<BaseVO> list = new ArrayList<BaseVO>();
			for(AdminCoreModel model : rows.getList()){
	...
	public String detail(@PathVariable(value = "id") String id){
		try {
			AdminCoreModel model = adminCoreService.selectById(id);
			BaseVO vo = new BaseVO();
