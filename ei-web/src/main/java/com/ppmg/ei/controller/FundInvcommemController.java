...
@Scope("prototype")
@Api(tags={"XXX接口"})
public class FundInvcommemController extends BaseAction{

	/*protected final Logger logger = LoggerFactory.getLogger(this.getClass());

	@Reference
	private FundInvcommemService fundInvcommemService;
	...
	public String list(@RequestParam("length") int length, @RequestParam("currPage") int currPage){
		try {
			...
			PageInfo<FundInvcommemModel> rows = fundInvcommemService.selectListPage(searchCondition);
			List<BaseVO> list = new ArrayList<BaseVO>();
			for(FundInvcommemModel model : rows.getList()){
	...
	public String detail(@PathVariable(value = "id") String id){
		try {
			FundInvcommemModel model = fundInvcommemService.selectById(id);
			BaseVO vo = new BaseVO();
			BeanUtils.copyProperties(vo, model);
