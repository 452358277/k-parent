...
public class AdminFundController extends BaseAction{
	...
	public String list(@RequestParam("length") int length, @RequestParam("currPage") int currPage){
		try {
			...
			PageInfo<AdminFundModel> rows = adminFundService.selectListPage(searchCondition);
			List<BaseVO> list = new ArrayList<BaseVO>();
			for(AdminFundModel model : rows.getList()){
	...
	public String detail(@PathVariable(value = "id") String id){
		try {
			AdminFundModel model = adminFundService.selectById(id);
			BaseVO vo = new BaseVO();
